...
class DownloadSync {
    ...
    async syncFile(file, localPath) {
        ...
        if(file.mimeType.startsWith("application/vnd.google-apps.")) {
            localPath = localPath + '.pdf'; // current status: export all google documents as PDF
        }
        ...
        if(!await this.checkFileExists(localPath)) {
            await this.downloadFile(file.id, localPath, file.mimeType);
        } else {
    ...
    //download file
    downloadFile(id, savePath, mimeType) {
        return new Promise(async (resolve) => {
            ...
            if(mimeType.startsWith("application/vnd.google-apps.")) {
                res = await this.drive.files.export(
                    {fileId: id, alt: 'media', mimeType: 'application/pdf'},
                    // todo: allow other exports: https://developers.google.com/drive/api/v3/ref-export-formats,
